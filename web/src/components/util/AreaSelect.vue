<template>
  <div class="components-util-areaselect">
      <el-select
         v-model="provinceId"
         placeholder="请选择省份"
         @change="change('pChange',$event)">
        <el-option
  ...
  export default {
    ...
    computed: {
      ...
      citys: function(){
        if(this.provinceId != ''){
          let selectArray = []
          return selectArray = [...this.areaData[this.provinceId]['children']]
        }else{
          return []
        }
      },
      provinceName: function(){
        if(this.provinceId != ''){
          return this.areaData[this.provinceId]['name']
        }else{
          return ''
        }
      },
      cityName: function(){
        if(this.provinceId != '' && this.cityId != ''){
          let datas = this.areaData[this.provinceId]['children']
    ...
    watch:{
      provinceCn:function(cur){
        this.provinceId = this.provinceNameBack()
        this.cityId = this.cityNameBack()
        this.change()
      }
    },
    methods: {
      ...
      provinceNameBack: function(){
        let data = bankSelectorData()
        let newData = Object.keys(data)
        for(let i=0;i<newData.length;i++){
          if(data[newData[i]].name == this.provinceCn){
            return data[newData[i]].id
          }
        }
        return ''
      },
      cityNameBack: function(){
        let data = this.citys
        for(let item of data){
          if(item.name == this.cityCn){
            return item.id
          }
        }
        return ''
